<?php
    ...
    de creation,modification,supresion de cours du point de vue logique
   */
...
$password = "";
$database = "m1_edt";
$validation = false;// si on peut mettre ou non 
$validation_modification = false; // si on peut mettre ou non le cours modifier 
$heure_ue = null;
...
if (!empty($_POST['id_ev'])) {
    //note: on fait deux modification de ue pour l'event present et celui quon met
    //test de modification chevauchement 
    test_de_modification();
    //if le test est valide

    if ($validation_modification) {
        test_de_modification_heure();//update de l'heure ue 
...
// if on a depasser notre cota d'heure on ne l'ajoute pas 
{
...
// dans cette fonction on test le chevauchement 
// au niveau de tout les parametre (prof,salle,groupe)
// les trois parametre doivent etre disponible 
{
    // soit si c'est une salle un prof ou un groupe
